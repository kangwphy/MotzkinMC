...
# --- Pm Operator ---
const PM_KEY = :Pm_vals
register_derived_quantity!(PM_KEY, (config, S; m) -> (abs.(config) .== m))

# --- Nm Operator ---
const NM_KEY = :Nm_vals
register_derived_quantity!(NM_KEY, (config, S; m) -> (config .== m))
...
register_observable!("pm_expect", (L, tmea; m, kwargs...) -> begin
    key_kwargs = (m=m,) # Package parameters into a NamedTuple
    filename = "pm$(m)_expect.dat"
    header = "m\tPm_Avg\tPm_Err"
    xaxis = [m]
    return GeneralExpect(filename, header, xaxis, PM_KEY; key_kwargs = key_kwargs)
end)

register_observable!("pm_tcorr", (L, tmea; m, kwargs...) -> begin
    final_m1 = m1 !== nothing ? m1 : m
    final_m2 = m2 !== nothing ? m2 : m
    if final_m1 === nothing || final_m2 === nothing; error("nm_tcorr requires 'm' or ('m1', 'm2')"); end
    ...
    header = "t\tC_Pm$(m)_Avg\tC_Pm$(m)_Err"
    # Call the convenience constructor for autocorrelation
    return GeneralTCorr(filename, header, tmea, PM_KEY , PM_KEY, L, Int; kwargs_t = kwargs_t, kwargs_0 = kwargs_0)
end)
...
register_observable!("nm_expect", (L, tmea; m, kwargs...) -> begin
    ...
    xaxis = [m]
    return GeneralExpect(filename, header, xaxis, NM_KEY; key_kwargs = key_kwargs)
end)
...
register_observable!("nm_tcorr", (L, tmea; m=nothing, m1=nothing, m2=nothing, kwargs...) -> begin
    ...
    header = "t\tC_Nm$(final_m1)m$(final_m2)_Avg\tC_Nm$(final_m1)m$(final_m2)_Err"
    
    # Call the main constructor for cross-correlation
    return GeneralTCorr(filename, header, tmea, NM_KEY, NM_KEY, L, Int; kwargs_t = kwargs_t, kwargs_0 = kwargs_0)
end)
