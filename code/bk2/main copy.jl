...
function main()
    ...
    # push!(observables_to_run, OExpectation())
    # if S < 2
    #     if rank == 0; println("Warning: C operator measurements require S >= 2. Skipping."); end
    # else
    #     if run_all || "c_exp" in modes
    #         push!(observables_to_run, CExpectation())
    #     end
    # end
    
    # obs = create_observable("c_expect", L, []) # Pass empty tmea for now
    # push!(observables_to_run, obs)
    ...
    # push!(observables_to_run, OOTCorr(tmea))
    if S >= 2
        # push!(observables_to_run, CCTCorr(tmea))
        obs = create_observable("cc_tcorr", L, tmea)
        @show obs
        push!(observables_to_run, obs)
    end
